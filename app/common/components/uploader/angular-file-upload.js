 ...
 *
 * Base on https://github.com/nervgh/angular-file-upload.
 * The angular file upload module
 * @author: nerv
 * @version: 0.5.7, 2014-05-23
 */
var uploader = angular.module('angularFileUpload', []);

uploader.factory('$fileUploader', ['$compile', '$rootScope', '$http', '$window', '$log', function ($compile, $rootScope, $http, $window, $log) {
    'use strict';
    ...
    Uploader.prototype = {
        ...
        addToQueue: function (items, options) {
            ...
            angular.forEach(list, function (file) {
                ...
                if (isValid) {
                    ...
                    this.trigger('whenaddingfilefailed', item, errMsg);
                }
            }, this);
        ...
        /**
         * Add by JiTing, 20140710
         * Call server API to delete uploaded file.
         * @param fileUrl
         * @param successCallback
         * @param errorCallback
         */
        removeUploadedFile: function (fileUrl, successCallback, errorCallback) {
            var mark = this.url.indexOf("?") > 0 ? "&" : "?";
            $http.get(this.url + mark + "action=delete&url=" + fileUrl)
                .success(function (data, status, headers, config) {
                    if (angular.isFunction(successCallback)) {
                        successCallback(data, status);
                    }
                }).error(function (data, status, headers, config) {
                    $log.error("Remove file failed: " + fileUrl, data);
        ...
        _xhrTransport: function (item) {
            ...
            xhr.onload = function () {
                ...
                var event = that._isSuccessCode(xhr.status) && that._isSuccessResponse(response) ? 'success' : 'error';
            ...
            // In php class, we use the following express to determine if the request is AJAX call, can response individually:
            // if (($_SERVER['HTTP_X_REQUESTED_WITH']) == 'XMLHttpRequest' )
            // Usually, the variable was handled by jQuery, here, we need set it by ourselves.
         ...
         * Checks whether the PHP upload response successful.
         * @param response
         * @private
         * Added by JiTing, 20140709
         */
        _isSuccessResponse: function (response) {
            return angular.isObject(response) && response.status == 'OK';
        },
    ...
    Item.prototype = {
        ...
        remove: function () {
            var item = this;
            item.uploader.removeUploadedFile(item.fileUrl, function () {
                if (item.uploader) {
...
uploader.directive('ngFileDrop', ['$fileUploader', function ($fileUploader) {
    'use strict';
...
uploader.directive('ngFileOver', function () {
    return {
...
uploader.directive('ngFileSelect', ['$fileUploader', function ($fileUploader) {
    return {
        link: function (scope, element, attributes) {
 ...
 */
uploader.directive('ngThumb', ['$window', function ($window) {
    var helper = {
    ...
    return {
        ...
        link: function (scope, element, attributes) {
            ...
            function onLoadImage() {
                ...
                canvas.attr({style: 'background-color: #f3f3f3;-webkit-box-shadow: 3px 3px 3px 0 #e3e3e3;-moz-box-shadow: 3px 3px 3px 0 #e3e3e3;box-shadow: 3px 3px 3px 0 #e3e3e3;border: 1px solid #c3c3c3;margin: 6px 0 0 0;'});
