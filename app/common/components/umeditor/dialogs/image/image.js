...
//                }
            ...
            callback: function (editor, $w, url, state) {
                ...
                if (state == "OK") {
                    //显示图片计数+1
                    Upload.showCount++;
                    // Check return url, if begin with http, for example BCS url, needn't add imagePath before. By JiTing, 20140725
                    var uploadedUrl = url.indexOf("http://") == 0 ? url : editor.options.imagePath + url;
                    var $img = $("<img src='" + uploadedUrl + "' class='edui-image-pic' />"),
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<input style="filter: alpha(opacity=0);" class="edui-image-file" type="file" hidefocus accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
            '</form>' +
        ...
        config: function (sel) {
            ...
            // set type=ajax, so demo upload php will return json data instead of iframe. By JiTing, 20140725.
            url = url + (url.indexOf("?") == -1 ? "?" : "&") + "type=ajax&editorid=" + me.editor.id;//初始form提交地址;
            // set file upload controller name according to editor configuration. By JiTing, 20140725.
            $(".edui-image-file").attr("name", me.editor.options.imageFieldName);
        ...
        uploadComplete: function (r) {
            ...
            try {
                var json = eval('(' + r + ')');
                Base.callback(me.editor, me.dialog, json.url, json.status);
            } catch (e) {
        ...
        submit: function (callback) {
            ...
            var me = this,
                input = $('<input style="filter: alpha(opacity=0);" class="edui-image-file" type="file" hidefocus="" name="' + me.editor.options.imageFieldName + '" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp">'),
                input = input[0];
